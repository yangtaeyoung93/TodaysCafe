...
<table >
   ...
   <c:if test="${login_info.admin eq 'y'}">
   		<a class="btn-fill" onclick="location='update?id=${vo.id}'"><img title="수정" src="img/eraser.png" style="width:20px"></a>&nbsp;
...
<table>
   ...
   <td>
   <a onclick="location='detail?id=${vo.next_idx}&read=1'">${vo.nexttitle}<img src="img/top-arrow.png" style="width: 18px" align="right"></a>
   <c:if test="${vo.next_idx eq 0}">다음 글이 없습니다</c:if>
   </td>
   </tr>
   <tr>
   <th align="left">이전글</th>
   <td ><a onclick="location='detail?id=${vo.pre_idx}&read=1'">${vo.pretitle}<img src="img/down-arrow.png" style="width: 18px" align="right"></a>
   <c:if test="${vo.pre_idx eq 0}">이전 글이 없습니다</c:if></td>
   
   </tr>
</table>
<br><br>
   <form id="detail" method="get" action="list">
